...
class Administrador extends Funcionario{
	...
	public function insert(){
		try{
			$stmt = $this->conn->prepare("INSERT INTO administrador(funcionario_id, nome_usuario) VALUES(:funcionario_id, :nome_usuario)");
	...
	public function edit(){
		try{
			$stmt = $this->conn->prepare("UPDATE administrador SET nome_usuario = :nome_usuario WHERE funcionario_id = :funcionario_id");
			$stmt->bindParam(":nome_usuario", $this->nome_usuario);
			$stmt->bindParam(":funcionario_id", $this->funcionario_id);
			$stmt->execute();
			return 1;
